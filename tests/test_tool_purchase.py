...
def _capture_tool_fn():
    from quickbooks_mcp.tools.purchase import register
...
@pytest.mark.asyncio
async def test_list_purchases_returns_paginated_results():
...
# ---------------------------------------------------------------------------
# 3. create does NOT require any ref (vendor_ref optional)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_create_without_refs_does_not_raise():
    purch = _make_tx_obj(id_="701")
    client = _make_client(return_value=purch)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        mock_cls.return_value = purch
        mock_cls_factory.return_value = mock_cls
...
# ---------------------------------------------------------------------------
# 8. vendor_ref is optional (accepted but not required)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_create_with_optional_vendor_ref():
    purch = _make_tx_obj(id_="702")
    client = _make_client(return_value=purch)
