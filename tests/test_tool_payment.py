...
def _capture_tool_fn():
    from quickbooks_mcp.tools.payment import register
...
# ---------------------------------------------------------------------------
# 1. list payments returns paginated results
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_payments_returns_paginated_results():
    payments = [_make_tx_obj(id_=str(i), total=float(100 * i)) for i in range(3)]
    client = _make_client(return_value=payments)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        mock_cls.filter.return_value = payments
...
async def test_create_requires_customer_ref():
    ...
    with patch("quickbooks_mcp.server.get_client") as mock_get_client:
        mock_get_client.return_value = _make_client()
        with pytest.raises(
            ToolError, match="customer_ref.*required.*creating a payment"
        ) as exc_info:
...
# ---------------------------------------------------------------------------
# 4. void with preview=True returns preview
# ---------------------------------------------------------------------------
...
async def test_void_preview_true_returns_preview():
    ...
    pmt = _make_tx_obj(id_="90", total=400.00)
    client = _make_client(return_value=pmt)
    ctx = _make_ctx(client)

    with patch("quickbooks_mcp.server.get_client", return_value=client):
        response = await tool_fn(ctx=ctx, operation="void", id="90")
...
# ---------------------------------------------------------------------------
# 5. void with preview=False executes
# ---------------------------------------------------------------------------
...
async def test_void_preview_false_actually_voids():
    ...
    pmt = _make_tx_obj(id_="91")
    pmt.void.return_value = None
    client = _make_client(return_value=pmt)
    ctx = _make_ctx(client)

    with patch("quickbooks_mcp.server.get_client", return_value=client):
        response = await tool_fn(ctx=ctx, operation="void", id="91", preview=False)
...
# ---------------------------------------------------------------------------
# 10. payment has no delete, send, or pdf operations
# ---------------------------------------------------------------------------


def test_tool_signature_has_no_delete_send_pdf():
    """Payment supports only list/get/create/update/void/search."""
    import typing
    ...
    assert "delete" not in literal_args
    assert "send" not in literal_args
    assert "pdf" not in literal_args


# ---------------------------------------------------------------------------
# 11. payment has no email parameter
# ---------------------------------------------------------------------------
...
@pytest.mark.asyncio
async def test_create_payment_with_customer_ref():
    created = _make_tx_obj(id_="200", total=500.00)
    client = _make_client(return_value=created)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        obj_instance = _make_tx_obj(id_="200", total=500.00)
...
@pytest.mark.asyncio
async def test_void_requires_id():
    tool_fn = _capture_tool_fn()
    assert tool_fn is not None
    ...
    with patch("quickbooks_mcp.server.get_client") as mock_get_client:
        mock_get_client.return_value = _make_client()
        with pytest.raises(ToolError, match="'id' is required"):
            await tool_fn(ctx=ctx, operation="void", id=None)
