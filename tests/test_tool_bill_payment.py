"""Tests for qbo_bill_payment tool."""
...
def _make_tx_obj(
    ...
    obj.to_dict.return_value = {
        ...
        "TotalAmt": total,
        "TxnDate": "2026-02-10",
        "VendorRef": {"value": "40", "name": "Supplier Co"},
    }
    return obj
...
def _capture_tool_fn():
    from quickbooks_mcp.tools.bill_payment import register
...
# ---------------------------------------------------------------------------
# 1. list bill payments
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_bill_payments_returns_paginated_results():
    items = [_make_tx_obj(id_=str(i)) for i in range(4)]
...
# ---------------------------------------------------------------------------
# 3. create does NOT require vendor_ref or customer_ref
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_create_without_refs_does_not_raise():
    """BillPayment create doesn't require vendor_ref or customer_ref."""
    bp = _make_tx_obj(id_="301")
    client = _make_client(return_value=bp)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        mock_cls.return_value = bp
        mock_cls_factory.return_value = mock_cls
...
# ---------------------------------------------------------------------------
# 5. void with preview=True returns preview
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 7. protected keys in extra raises error
# ---------------------------------------------------------------------------
