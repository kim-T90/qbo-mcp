...
def _capture_tool_fn():
    from quickbooks_mcp.tools.estimate import register
...
# ---------------------------------------------------------------------------
# 1. list estimates returns paginated results
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_estimates_returns_paginated_results():
    estimates = [_make_tx_obj(id_=str(i), total=float(100 * i)) for i in range(3)]
    client = _make_client(return_value=estimates)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        mock_cls.filter.return_value = estimates
...
async def test_create_requires_customer_ref():
    ...
    with patch("quickbooks_mcp.server.get_client") as mock_get_client:
        mock_get_client.return_value = _make_client()
        with pytest.raises(
            ToolError, match="customer_ref.*required.*creating an estimate"
        ) as exc_info:
...
async def test_delete_preview_true_returns_preview():
    ...
    est = _make_tx_obj(id_="90", total=400.00)
    client = _make_client(return_value=est)
...
async def test_delete_preview_false_actually_deletes():
    ...
    est = _make_tx_obj(id_="91")
    est.delete.return_value = None
    client = _make_client(return_value=est)
    ctx = _make_ctx(client)
...
# ---------------------------------------------------------------------------
# 11. send estimate with email works
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_send_estimate_with_email():
...
# ---------------------------------------------------------------------------
# 12. create estimate with customer_ref
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_create_estimate_with_customer_ref():
    ...
    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        ...
        response = await tx_create(
            ...
            customer_ref="10",
            vendor_ref=None,
            line_item_dicts=[{"amount": 150.00, "description": "Estimate line"}],
            memo="Test estimate",
            due_date="2026-02-28",
            extra_dict={},
...
# ---------------------------------------------------------------------------
# 13. estimate has no void operation
# ---------------------------------------------------------------------------


def test_tool_signature_has_no_void_operation():
    """Estimate does not support void — verify it's not in the Literal."""
    import typing
