"""Tests for qbo_refund_receipt tool."""
...
def _capture_tool_fn():
    from quickbooks_mcp.tools.refund_receipt import register
...
# ---------------------------------------------------------------------------
# 1. list refund receipts returns paginated results
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_list_refund_receipts_returns_paginated_results():
...
async def test_create_requires_customer_ref():
    ...
    with patch("quickbooks_mcp.server.get_client") as mock_get_client:
        mock_get_client.return_value = _make_client()
        with pytest.raises(
            ToolError, match="customer_ref.*required.*creating a refund_receipt"
        ) as exc_info:
...
async def test_delete_preview_true_returns_preview():
    ...
    rr = _make_tx_obj(id_="90", total=400.00)
    client = _make_client(return_value=rr)
...
async def test_delete_preview_false_actually_deletes():
    ...
    rr = _make_tx_obj(id_="91")
    rr.delete.return_value = None
    client = _make_client(return_value=rr)
    ctx = _make_ctx(client)
...
# ---------------------------------------------------------------------------
# 10. refund receipt has no void, send, or pdf operations
# ---------------------------------------------------------------------------


def test_tool_signature_has_no_void_send_pdf():
    """RefundReceipt supports only list/get/create/update/delete/search."""
    import typing
...
# ---------------------------------------------------------------------------
# 11. refund receipt has no email parameter
# ---------------------------------------------------------------------------
...
@pytest.mark.asyncio
async def test_create_refund_receipt_with_customer_ref():
    created = _make_tx_obj(id_="200", total=75.00)
    client = _make_client(return_value=created)

    with patch("quickbooks_mcp.tools._base.get_tx_class") as mock_cls_factory:
        mock_cls = MagicMock()
        obj_instance = _make_tx_obj(id_="200", total=75.00)
        ...
        response = await tx_create(
            ...
            customer_ref="10",
            vendor_ref=None,
            line_item_dicts=[{"amount": 75.00, "description": "Refund line"}],
            memo=None,
            due_date=None,
            extra_dict={},
