"""qbo_journal_entry tool — manage QuickBooks Online journal entries."""
...
_TX_TYPE = "journal_entry"
_ENTITY_TYPE = "JournalEntry"

_TOOL_DESCRIPTION = (
    f"{PREVIEW_HINT} "
    "Manage QuickBooks Online journal entries. "
    "Create, list, update, delete, or search journal entries. "
    "line_items use JournalEntryLineDetail: "
    '[{"amount": 100.0, "PostingType": "Debit", '
    '"AccountRef": {"value": "1"}}]'
    " — see qbo_help(topic='line_items', entity='journal_entry') for full schema. "
    f"Search uses IDS query syntax — {IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['journal_entry']} "
    f"{ERROR_SHAPE_HINT}"
)


def register(mcp: FastMCP) -> None:
    """Register the qbo_journal_entry tool on *mcp*."""
    from quickbooks_mcp.server import get_client  # noqa: PLC0415

    @mcp.tool(
        name="qbo_journal_entry",
    ...
    async def qbo_journal_entry(
        ...
        """Create, read, update, delete, or search QBO journal entries.
