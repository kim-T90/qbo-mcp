"""qbo_customer tool — manage QuickBooks Online customers."""
...
def _get_class():
    """Lazily import and return the python-quickbooks Customer class."""
...
_TOOL_DESCRIPTION = (
    ...
    "not allow permanent deletion of customers. "
    f"Search uses IDS query syntax — {IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['customer']} "
    f"{ERROR_SHAPE_HINT}"
)
...
def register(mcp: FastMCP) -> None:
    """Register the qbo_customer tool on *mcp*."""
    ...
    async def qbo_customer(
        ...
        """Create, read, update, deactivate, or search QuickBooks Online customers.

        Args:
            operation: Action to perform. One of:
                - list: Paginated list of customers.
                - get: Fetch a single customer by id.
                - create: Create a new customer with supplied fields.
                - update: Merge supplied fields onto an existing customer (id required).
                - deactivate: Set Active=False on a customer (id required).
                - search: Run a raw IDS query against the Customer table.
