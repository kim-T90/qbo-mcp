"""qbo_estimate tool — manage QuickBooks Online estimates."""
...
_TOOL_DESCRIPTION = (
    f"{PREVIEW_HINT} "
    "Manage QuickBooks Online estimates. "
    "Create, list, update, delete, email, or download as PDF. "
    "create requires customer_ref (customer ID from qbo_customer). "
    'line_items: [{"amount": 100.0, "description": "...", "item_ref": "1"}]'
    " — see qbo_help(topic='line_items', entity='estimate') for full schema. "
    f"Search uses IDS query syntax — {IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['estimate']} "
    "Typical flow: qbo_customer(list) -> get customer ID -> "
    f"qbo_estimate(create). {ERROR_SHAPE_HINT}"
)


def register(mcp: FastMCP) -> None:
    """Register the qbo_estimate tool on *mcp*."""
    ...
    async def qbo_estimate(
        ctx: Context,
        operation: Literal["list", "get", "create", "update", "delete", "send", "pdf", "search"],
        id: str | None = None,
        ...
        """Create, read, update, delete, send, or search QuickBooks Online estimates.

        Args:
            operation: Action to perform.
            id: QBO entity ID (numeric string, e.g. '42') — required
                for get, update, delete, send, pdf.
            ...
            memo: Private note on the estimate.
            due_date: Expiration date in YYYY-MM-DD format.
        ...
        if operation in ("get", "update", "delete", "send", "pdf") and not id:
        ...
        if operation == "create" and not customer_ref:
            raise ToolError(
                "'customer_ref' is required when creating an estimate. "
