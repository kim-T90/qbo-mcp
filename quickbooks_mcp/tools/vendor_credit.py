"""qbo_vendor_credit tool — manage QuickBooks Online vendor credits."""
...
_TX_TYPE = "vendor_credit"
_ENTITY_TYPE = "VendorCredit"

_TOOL_DESCRIPTION = (
    f"{PREVIEW_HINT} "
    "Manage QuickBooks Online vendor credits. "
    "Create, list, update, delete, or search vendor credits. "
    "create requires vendor_ref (vendor ID). "
    'line_items: [{"amount": 100.0, "description": "..."}]'
    " — see qbo_help(topic='line_items', entity='vendor_credit') for full schema. "
    f"Search uses IDS query syntax — {IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['vendor_credit']} "
    f"{ERROR_SHAPE_HINT}"
)


def register(mcp: FastMCP) -> None:
    """Register the qbo_vendor_credit tool on *mcp*."""
    from quickbooks_mcp.server import get_client  # noqa: PLC0415

    @mcp.tool(
        name="qbo_vendor_credit",
    ...
    async def qbo_vendor_credit(
        ...
        """Create, read, update, delete, or search QBO vendor credits.
        ...
        if operation == "create" and not vendor_ref:
            raise ToolError(
                "'vendor_ref' is required when creating a "
                "vendor credit. "
        ...
        try:
            if operation == "list":
                ...
                return await tx_create(
                    ...
                    line_items,
                    memo,
                    None,  # VendorCredit has no DueDate
                    extra_dict,
                    response_format,
                )
