"""qbo_employee tool — manage QuickBooks Online employees."""
...
def _get_class():
    """Lazily import and return the python-quickbooks Employee class."""
...
_TOOL_DESCRIPTION = (
    "Manage QuickBooks Online employees. "
    ...
    "not allow permanent deletion of employees. "
    f"Search uses IDS query syntax — {IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['employee']} "
    f"{ERROR_SHAPE_HINT}"
)
...
def register(mcp: FastMCP) -> None:
    """Register the qbo_employee tool on *mcp*."""
    ...
    async def qbo_employee(
        ...
        """Create, read, update, deactivate, or search QuickBooks Online employees.

        Args:
            operation: Action to perform. One of:
                - list: Paginated list of employees.
                - get: Fetch a single employee by id.
                - create: Create a new employee with supplied fields.
                - update: Merge supplied fields onto an existing employee (id required).
                - deactivate: Set Active=False on an employee (id required).
                - search: Run a raw IDS query against the Employee table.
