"""qbo_deposit tool — manage QuickBooks Online deposits."""
...
_TX_TYPE = "deposit"
_ENTITY_TYPE = "Deposit"

_TOOL_DESCRIPTION = (
    f"{PREVIEW_HINT} "
    "Manage QuickBooks Online deposits. "
    "Create, list, update, delete, or search deposits. "
    "create requires deposit_to_account_ref (account ID from qbo_account). "
    'line_items: [{"amount": 100.0, "description": "..."}]'
    " — see qbo_help(topic='line_items', entity='deposit') for full schema. "
    "Search uses IDS query syntax (not SQL) — "
    f"{IDS_QUERY_RULES} "
    f"{SEARCH_EXAMPLES['deposit']} "
    f"{ERROR_SHAPE_HINT}"
)


def register(mcp: FastMCP) -> None:
    """Register the qbo_deposit tool on *mcp*."""
    ...
    async def qbo_deposit(
        ...
        """Create, read, update, delete, or search QBO deposits.

        Args:
            operation: Action to perform.
            id: QBO entity ID (numeric string, e.g. '42') — required
                for get, update, delete.
            deposit_to_account_ref: Target bank account ID (required
                for create). Use qbo_account(operation='list') to find IDs.
        ...
        # Promote first-class param into extra
        if deposit_to_account_ref is not None:
            extra_dict["DepositToAccountRef"] = {"value": deposit_to_account_ref}
